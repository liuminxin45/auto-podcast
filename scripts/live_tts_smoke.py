...
def run_live_tts_smoke(engine: str, output_dir: Path) -> dict:
    ...
    if not state.get("final_audio_path") or not Path(state["final_audio_path"]).exists():
...
def _build_tts_config(engine: str, voice_dir: Path) -> TTSConfig:
    if engine in {"openai-compatible", "openai", "openai-audio"}:
...
def main() -> int:
    ...
    print(f"audio: {state.get('final_audio_path', '')}")
    return 0

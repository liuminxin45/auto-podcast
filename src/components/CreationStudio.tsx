...
export default function CreationStudio({
  ...
  useEffect(() => {
    ...
    setSelectedFactIds(initialIds.length > 0 ? initialIds : nextFacts.slice(0, 4).map(fact => fact.id))
  ...
  const warnings = useMemo(() => {
    ...
    if (selectedFacts.length < 3) result.push('早报条目少于 3 条')
    if (selectedFacts.length > 5) result.push('早报条目超过 5 条')
  ...
  const regenerateFacts = useCallback(async () => {
    ...
    setFacts(nextFacts)
    setSelectedFactIds(nextFacts.slice(0, 4).map(fact => fact.id))
  ...
  const toggleFact = useCallback((factId: string) => {
    setSelectedFactIds(prev => {
      ...
      if (prev.length >= 5) {
        message.warning({ content: '单期早报最多选择 5 条事实卡片', duration: 1.5, style: { marginTop: 60 } })
        return prev
      }
      return [...prev, factId]
    })
    ...
    <div style={{
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          {onBackToOrganize && (
            <Button icon={<ArrowLeftOutlined />} onClick={onBackToOrganize}>返回整理</Button>
          )}
          <div>
            <div style={{ fontSize: 16, fontWeight: 700 }}>事实卡片与早报编排</div>
            <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>
              单人新闻早报主路径：素材到 FactCard，再到 3-5 条通勤早咖啡 rundown
            </div>
          </div>
        </div>
      ...
      <main style={{
        ...
        minHeight: 0,
        display: 'grid',
        gridTemplateColumns: '320px minmax(420px, 1fr) 360px',
      }}>
        <section style={{ borderRight: '1px solid var(--border-color)', padding: 16, overflow: 'auto' }}>
          <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 10 }}>素材 inbox</div>
        ...
        <section style={{ padding: 18, overflow: 'auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <div>
              <div style={{ fontSize: 14, fontWeight: 700 }}>FactCard board</div>
              <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>选择 3-5 条进入本期早报</div>
            </div>
            <Tag bordered={false} color={selectedFacts.length >= 3 && selectedFacts.length <= 5 ? 'green' : 'orange'}>
              已选 {selectedFacts.length}/5
            </Tag>
          </div>
          {warnings.length > 0 && (
            <div style={{
            ...
            {facts.map(fact => {
              ...
              return (
                ...
                >
                  ...
                  <Tooltip title={fact.source_url || '无 URL'}>
                    <div style={{ marginTop: 10, color: 'var(--text-tertiary)', fontSize: 11, wordBreak: 'break-all' }}>
        ...
        <section style={{ borderLeft: '1px solid var(--border-color)', padding: 16, overflow: 'auto' }}>
          <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 10 }}>本期 rundown</div>
          ...
          {selectedFacts.map((fact, index) => (
            <div key={fact.id} style={{
              ...
              <div style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>新闻 {index + 1}</div>

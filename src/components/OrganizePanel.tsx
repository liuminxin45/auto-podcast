...
type Priority = 'primary' | 'important' | 'backup'
...
// ============================================================
// Category detection (reused from DiscoverPanel style)
// ============================================================
...
function getQualitySignals(item: ContentItem, allItems: ContentItem[], topic?: string): Array<{ icon: string; text: string }> {
  const signals: Array<{ icon: string; text: string }> = []

  // Multi-source confirmation: check if similar titles exist
  ...
  const reliableSources = ['reuters', 'bloomberg', 'techcrunch', 'theverge', 'arxiv', 'nature', '新华社', '人民日报', '财新']
  if (item.source && reliableSources.some(s => (item.source || '').toLowerCase().includes(s))) {
...
const PRIORITY_CONFIG: Record<Priority, { label: string; barColor: string; bgColor: string; tagBg: string; tagColor: string }> = {
  primary: { label: '主线候选', barColor: '#2563eb', bgColor: '#eff6ff', tagBg: '#dbeafe', tagColor: '#1d4ed8' },
  important: { label: '重要', barColor: '#06b6d4', bgColor: '#ffffff', tagBg: '#cffafe', tagColor: '#0891b2' },
  backup: { label: '备用', barColor: '#e5e7eb', bgColor: '#ffffff', tagBg: '#f3f4f6', tagColor: '#6b7280' },
}

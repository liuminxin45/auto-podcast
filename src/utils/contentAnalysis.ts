...
// ============================================================
// Advanced AI-Assisted Analysis (Local Algorithms)
// ============================================================
...
const EXTENDED_RELIABLE_SOURCES = [
  'reuters', 'bloomberg', 'techcrunch', 'theverge', 'arxiv', 'nature',
...
export function analyzePriorityHints(
  ...
  for (const item of items) {
    ...
    if (item.source && EXTENDED_RELIABLE_SOURCES.some(s => (item.source || '').toLowerCase().includes(s))) {
      score += 15
      reasons.push('来源可靠')
    }
...
export function detectDuplicatesAndNoise(
  ...
  for (const item of items) {
    if (duplicates.has(item._id)) continue
